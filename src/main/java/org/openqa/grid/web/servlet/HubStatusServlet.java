...
public class HubStatusServlet extends RegistryBasedServlet {
	...
	protected void process(HttpServletRequest request,
		...
		try {
		    ...
		    getRegistry().getGridMonitorMBean().handleHubStatusRequest(theJson,res.toString());
	...
	private JsonObject getResponse(HttpServletRequest request)
		...
		try {
			if (request.getInputStream() != null) {
				...
				JsonArray keys = requestJSON != null ? requestJSON.get(
						"configuration").getAsJsonArray() : null;
    ...
    theJson = s.toString();
    if (!"".equals(theJson)) {
      requestJSON = new JsonParser().parse(theJson).getAsJsonObject();
    }
    return requestJSON;
  }
